 ...
 * The order comes from STAT_CARDS in config.js. Each entry below says which icon to use, what the
...
const STAT_DEFS = {
  deaths: {
    ...
    avg: () => f1(average(RUNS.map(nonIntentionalDeaths))),
  },
  elytra: {
    icon: "s_elytra", name: () => T.statElytra,
    one: r => r.meta && r.meta.elytraCm != null ? (r.meta.elytraCm / 100000).toFixed(1) + " km" : "—",
    avg: () => { const a = average(RUNS.map(r => r.meta && r.meta.elytraCm != null ? r.meta.elytraCm / 100000 : null)); return a != null ? a.toFixed(1) + " km" : "—"; },
  },
  skulls: {
    icon: "s_skulls",
    ...
    avg: () => { const a = RUNS.reduce((x, r) => { const q = derive(r).skullRate; return [x[0] + q.skulls, x[1] + q.kills]; }, [0, 0]); return a[0] ? "1 / " + (a[1] / a[0]).toFixed(1) : "—"; },
  },
  tntPerDebris: {
    icon: "s_tnt", icon2: "s_debris", name: () => T.statTntPerDebris,
    one: r => { const x = derive(r).tntPer; return x != null ? x.toFixed(1) : "—"; },
    avg: () => { const a = RUNS.reduce((x, r) => [x[0] + derive(r).tntD, x[1] + derive(r).debD], [0, 0]); return a[1] ? (a[0] / a[1]).toFixed(1) : "—"; },
  },
  shulkers: {
    icon: "s_shulker", name: () => T.statShulkers,
    one: r => num(r.tot.shulkerOpen),
    avg: () => { const a = average(RUNS.map(r => r.tot.shulkerOpen)); return a != null ? a.toFixed(0) : "—"; },
  },
  creepers: {
    icon: "s_creepers", name: () => T.statCreepers,
    one: r => num(r.tot.creepers),
    avg: () => { const a = average(RUNS.map(r => r.tot.creepers)); return a != null ? a.toFixed(0) : "—"; },
  },
};
